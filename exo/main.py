...
# Medusa options
parser.add_argument("--medusa-enable", action="store_true", help="Enable Medusa decoding for faster inference")
parser.add_argument("--medusa-heads", type=int, default=4, help="Number of Medusa heads to use")
parser.add_argument("--medusa-tree-size", type=int, default=5, help="Maximum tree size for Medusa decoding")
parser.add_argument("--medusa-candidates", type=int, default=5, help="Maximum number of candidates to consider in Medusa")
...
# Enable Medusa if requested
if args.medusa_enable:
  print("Enabling Medusa decoding for faster inference (experimental)")
  inference_engine.use_medusa(
    enable=True,
    heads=args.medusa_heads,
    tree_size=args.medusa_tree_size,
    candidates=args.medusa_candidates
  )
...
async def run_model_cli(node: Node, model_name: str, prompt: str):
  ...
  # Get the repository and model config
  repo = get_repo(shard.model_id, inference_class)
  from exo.models import model_cards
  model_config = model_cards.get(model_name, {})
  ...
  try:
    ...
    if "tokenizer.chat_template is not set" in str(e):
      print("Warning: Chat template not available for this model. Using raw prompt.")
      # For models without chat templates, we can use a simple format
      if hasattr(tokenizer, "eos_token"):
        prompt = f"USER: {prompt}\nASSISTANT:" + (tokenizer.eos_token or "")
    else:
      raise

  # Check if we should use Medusa decoding
  if args.medusa_enable:
    # Check if this is a Medusa model
    is_medusa = model_config.get("is_medusa", False)
    if is_medusa:
      from exo.inference.medusa_decoder import MedusaDecoder
      print(f"Enabling Medusa decoding with {args.medusa_heads} heads, tree size {args.medusa_tree_size}, and {args.medusa_candidates} candidates")
      print("Note: This is an experimental feature and may not work correctly for all models")
      
      # Normally we would create the MedusaDecoder here and use it for inference
      # This is a placeholder until full Medusa integration is implemented
      print("Medusa support is recognized but currently in experimental state")
    else:
      print("Warning: --medusa-enable flag used but the model is not a Medusa model")
